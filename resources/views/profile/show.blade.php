<x-app-layout>
    <div class="relative w-full bg-renews-noir-impure overflow-hidden flex flex-col items-center pb-6 font-sans">
        ...
        <div class="relative z-10 w-full px-6 pt-24 pb-12 mx-auto max-w-sm flex flex-col items-center">
            ...
            <div class="w-full bg-[#232222]/90 backdrop-blur-sm rounded-2xl p-6 mb-6 border border-white/5 shadow-xl">
                <div class="flex justify-between text-xs font-bold mb-3 text-gray-400 tracking-wider">
                    <span>XP ACTUEL</span>
                    <span class="text-renews-vert">{{ $user->current_xp ?? 0 }} / {{ $nextLevelThreshold }} XP</span>
                </div>
                
                <div class="w-full bg-renews-noir rounded-full h-4 border border-white/10 relative overflow-hidden shadow-inner">
                    <div class="bg-gradient-to-r from-renews-fonce to-renews-electric h-full rounded-full relative z-10 transition-all duration-1000" 
                         style="width: {{ $progressPercent }}%">
                    </div>
                </div>
            </div>

            {{-- STATS (Série & Classement) --}}
            <div class="w-full flex justify-between gap-4 mb-8">
                ...
                {{-- Carte 2 : Classement (MODIFIÉE) --}}
                <div class="bg-white rounded-2xl p-4 flex-1 flex flex-col items-center justify-center h-32 shadow-lg relative overflow-hidden">
                    {{-- Affiche "Top X" --}}
                    <div class="z-10 flex flex-col items-center">
                        <span class="text-lg font-bold text-gray-400 uppercase leading-none mb-1">Top</span>
                                            <div class="flex">
                        <span class="text-5xl font-black text-black leading-none">{{ $rank }}</span>

                        {{-- Icône Trophée --}}
                        <i class="fa-solid fa-trophy text-yellow-500 text-xl mt-1 z-10"></i>
                        </div> 
                    </div>
                    {{-- Déco fond --}}
                    <i class="fa-solid fa-trophy text-gray-100 text-9xl absolute -bottom-6 -right-6 -rotate-12 z-0"></i>
                </div>
            ...
            <div class="w-full space-y-4">
                <a href="{{ route('profile.edit') }}" class="block w-full py-4 rounded-xl bg-renews-vert text-renews-noir font-bold text-center uppercase tracking-wide hover:brightness-110 transition">
                    Modifier mon profil
                </a>

                <form method="POST" action="{{ route('logout') }}" class="w-full">
                    @csrf
                    <button type="submit" class="block w-full py-4 rounded-xl bg-renews-noir-impure text-white font-bold text-center uppercase tracking-wide hover:brightness-110 transition">
                        Déconnexion
                    </button>
                </form>
            </div>
