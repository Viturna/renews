...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Renews - Créer un compte</title>

        <!-- Fonts & Scripts (Comme sur la home) -->
    ...
    <body class="bg-renews-noir-impure text-white font-sans antialiased min-h-screen flex flex-col relative overflow-x-hidden">
        ...
        <div class="flex-1 flex flex-col items-center justify-center px-6 py-10 z-10 w-full max-w-md mx-auto">
            ...
            <h1 class="text-3xl font-medium mb-8 text-center tracking-tight mt-4">
                Créer un <span class="text-renews-vert font-accent italic text-4xl">compte</span>
            </h1>

            <!-- 3. BOUTON GOOGLE -->
            <a href="{{ route('social.redirect', 'google') }}" 
               class="w-full flex items-center justify-center bg-white text-gray-800 font-medium text-base py-3 px-4 rounded-l shadow-md hover:bg-gray-100 transition-transform active:scale-95 mb-8">
                <!-- SVG Google Officiel -->
            ...
            <form method="POST" action="{{ route('register') }}" class="w-full space-y-5">
                ...
                <!-- Nom d'utilisateur -->
                <div>
                    <label for="username" class="block text-white font-bold mb-2 text-sm pl-1">Nom d’utilisateur</label>
                    ...
                    <x-input-error :messages="$errors->get('username')" class="mt-2 text-red-400 font-bold text-sm" />
                </div>
                ...
                <!-- Confirmation -->
                <div>
                    <label for="password_confirmation" class="block text-white font-bold mb-2 text-sm pl-1">Confirmation</label>
                    <input id="password_confirmation" type="password" name="password_confirmation" required 
                           placeholder="Ton mot de passe" 
                           class="w-full rounded-xl border-none bg-white py-3.5 px-4 text-gray-800 placeholder-gray-400 focus:ring-4 focus:ring-renews-vert/50 transition-shadow">
                    <x-input-error :messages="$errors->get('password_confirmation')" class="mt-2 text-red-400 font-bold text-sm" />
                </div>
